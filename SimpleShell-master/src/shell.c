...
char* readline() {
    ...
    if ((str_len = getline(&line, &size, stdin)) == -1) {
        ...
        if (errno != 0) {
            printf("[ERROR] Не смог прочитать ввод\n");
        }
        free(line);
        printf("\n");
        return NULL;
...
char** split(char* line) {
    ...
    if (tokens == NULL) {
        printf("[ERROR] Не смог выделить память для буфера при разделении!\n");
        return NULL;
    }
    ...
    while (token != NULL) {
        ...
        // Если свободное место закончилось удваиваем память для буффера
...
//В эту функцию добавлять системные функции 
//ls
//cat
//Программа получает сигнал нажатия комбинации Ctrl+C и останавливает 
//запущенную программу, например, браузер.
//signal(SIGINT, onCtrlC);
//где onCtrlC функция, останавливающая другой процесс
...
///////////////////////////////////////////////////////////////////////////////////////////
//              Далее аналоги системных функций
///////////////////////////////////////////////////////////////////////////////////////////
